...
export function BuildCardVertical({ state, theme, lookups, tokens }: BuildCardVerticalProps): React.JSX.Element {
  ...
  /**
   * Zero items anywhere keeps the friendly "não montada ainda" message
   * (unchanged UX for a brand-new build). Any single filled slot — even
   * without a mainhand — switches to the real layout below, where each
   * individual slot (mainhand included) renders its own placeholder when
   * empty instead of the whole card hiding behind this message (ACM-092
   * AC#2/#3: a partially filled build, e.g. only a helmet chosen, must still
   * export showing that helmet).
   */
  const hasAnyItem = SLOT_ORDER.some((slot) => state.slots[slot] !== null);
      ...
      <div className="flex flex-col gap-6 p-10">
        ...
        {!hasAnyItem ? (
          <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
            ...
            <p className="text-[15px] italic" style={{ color: tokens.fgMuted }}>
              {state.name || "Sem nome"}
            </p>
          ...
          <>
            <div className="flex gap-6">
              {mainhand ? (
                <div className="relative flex size-36 shrink-0 items-center justify-center rounded-md" style={{ backgroundColor: tokens.surface2 }}>
                  <ItemIcon
