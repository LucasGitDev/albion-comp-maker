...
/**
 * One-level breadcrumb for the editor (AC#3 / doc-004 §3.1). "Minhas comps"
 * is a real `<Link href="/">`, never `router.back()` — `history.back()` is
 * unreliable when the user arrived via a direct link or a refresh, and AC#3
 * explicitly requires a way back that doesn't depend on browser history.
 */
export function Breadcrumb({ current }: BreadcrumbProps): React.JSX.Element {
  return (
