...
/**
 * Recoverable error state for `/builds` (ACM-110 AC#6), mirroring
 * `CompListErrorRetry`: `listMyBuilds()` failing must never silently
 * `redirect("/")` — it shows this message with a "Tentar de novo" action
 * that re-runs the Server Component via `router.refresh()`.
 */
export function BuildsListErrorRetry(): React.JSX.Element {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-[var(--color-border)] bg-[var(--color-surface)] py-16 text-center">
      <p className="text-base font-medium text-foreground">Não foi possível carregar suas builds</p>
